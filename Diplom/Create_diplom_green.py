# Create_diplom.py Copies a base-file and save one file 
...
folder = ''
documentnavn = 'Ironkids Diplom'
base_file = 'ironkids_green.docx'
subfolder = 'Diplom'
...
def edit_header(file_name, name):
    try:
        ...
        doc.tables[0].cell(0,0).text = name
        doc.tables[0].cell(0,0).paragraphs[0].runs[0].font.size = Pt(24)     
        doc.tables[0].cell(0,0).paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER

        # Add name on page two
        doc.tables[1].cell(0,0).text = name
        doc.tables[1].cell(0,0).paragraphs[0].runs[0].font.size = Pt(24)
        
        doc.save(file_name)
    except docx.opc.exceptions.PackageNotFoundError as docx_error:
        print(docx_error)
        print(f'*** ERROR at edit_header() - {file_name} not foundru or is all ready opened***')
...
def copy_frame(folder, base_file, new_path, name):
    ...
    pdf_file_path = foldername + f'\\{documentnavn} {dater().year} {name}.pdf'
    try:
